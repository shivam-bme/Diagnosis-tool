...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
      if (file.mimetype === "image/tiff" || "image/png" || "image/jpeg") {
        cb(null, true);
        cb(null, Date.now() + path.extname(file.originalname));

      } else {
        cb(new Error("Invalid file type. Only TIFF, png files are allowed."));
      }
    },
  });
  
  app.get('/',(req,res)=>{
    const filepath = path.join(__dirname,"","bone.html");
    const filecontent = fs.readFileSync(filepath,"utf-8"); 
    res.send(filecontent);   
  });

  const indexHtml = fs.readFileSync(path.join(__dirname, './bone.html'), 'utf-8');
  console.log('unlock',__dirname);
